...
Scans training/evaluation data to build compact vocabulary mappings,
...
def run_offline_remap(
    ...
    Args:
        ...
        scan_paths: Optional explicit list of paths to scan for vocabulary.
                    If None, scans train_positive/train + valid + test.
    """
    ...
    if scan_paths is None:
        ...
        scan_paths = [train_path, valid, test]

    logger.info(f"  Scanning {len(scan_paths)} splits for vocabulary...")
    ...
    # Step 8: Save remap_dict.pkl for potential cloud-side inference use
    remap_dict_path = os.path.join(output_dir, 'remap_dict.pkl')
    with open(remap_dict_path, 'wb') as f:
        pickle.dump(remap_dicts, f)
...
def remap_stage_output(
    ...
    Args:
        ...
        remap_dicts: Loaded from mapped/remap_dict.pkl
